...
class GameLogicClient(GameLogicBase):
    ...
    def get_state(self) -> GameState:
        ...
        response_value = response.json().get("game_state")
        return GameState(response_value)

    def drop_token(self, player, column) -> DropState:
        question = { "column": column, "player_id": player }
        response = requests.post( f"{self._url}/drop", json= question)
        response_value = response.json().get("drop_state")
        return DropState(response_value)
